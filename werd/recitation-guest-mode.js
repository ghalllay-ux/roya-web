// Self-contained guest voice recitation for Chrome on iPhone — v106
(function(){
  ...
  let session=[],idx=0,recorder=null,stream=null,chunks=[],busy=false,transcript='',analysis=null,rated=false,hintUsed=false,elapsed=0,startedAt=0,timer=null,active=false;
  ...
  function setSupport(){
    ...
    if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder){start.disabled=true;if(support)support.innerHTML='<span>⚠️</span><div><b>التسجيل الصوتي غير متاح في هذا الإصدار من Chrome.</b><br>حدّث Chrome ثم أعد المحاولة.</div>';return}
    start.disabled=false;start.textContent='ابدأ جلسة التسميع';if(support)support.innerHTML='<span>🎙️</span><div><b>جاهز للتسميع مباشرة بدون تسجيل ✓</b><br>اختر الآية واضغط بدء الجلسة. التسجيل الصوتي مؤقت للتحويل فقط ولا يُحفظ.</div>'
  }
  ...
  function startSession(){
    if(!directActive())return false;if(!navigator.mediaDevices?.getUserMedia||!window.MediaRecorder){notify('التسجيل الصوتي غير متاح في هذا الإصدار من Chrome');return true}
  ...
  async function startRecording(){
    if(busy||!active)return;busy=true;const mic=$('rtestMic');if(mic){mic.disabled=true;mic.classList.add('starting')}if($('rtestMicStatus'))$('rtestMicStatus').textContent='جاري تشغيل الميكروفون…';
  ...
  function stopRecording(discard=false){if(!recorder)return;recorder.__discard=discard;try{if(recorder.state==='recording')recorder.stop()}catch(_){}stopTimer()}
  ...
  async function processRecording(mime){
    ...
    try{const fd=new FormData();fd.append('audio',blob,'werd-recitation.m4a');const headers={};try{if(typeof SUPABASE_PUBLISHABLE_KEY!=='undefined')headers.apikey=SUPABASE_PUBLISHABLE_KEY}catch(_){}const res=await fetch(ENDPOINT,{method:'POST',headers,body:fd});const j=await res.json().catch(()=>({}));if(!res.ok||!j.text)throw new Error(j.code||j.error||`HTTP_${res.status}`);transcript=String(j.text).trim();if($('rtestLive'))$('rtestLive').innerHTML=transcript?`<span>${esc(transcript)}</span>`:'<span class="muted">لم يُلتقط نص واضح.</span>';idleMic(!!transcript);if(transcript)notify('تم تحويل التسميع إلى نص ✓')}
    catch(e){console.error('Werd guest transcription',e);transcript='';if($('rtestLive'))$('rtestLive').innerHTML='<span class="muted">تعذر التحويل. أعد المحاولة.</span>';idleMic(false);notify(String(e?.message||'').includes('QUOTA')?'خدمة التحويل الصوتي وصلت حدها مؤقتًا':'تعذر تحويل الصوت الآن، حاول مرة أخرى')}
    finally{busy=false}
  }
  ...
  function capture(e){
    const t=e.target?.closest?.('#rtestStart,#rtestMic,#rtestAnalyze,#rtestHintBtn,#rtestRetry,#rtestNext,#rtestAgain,[data-rrate]');if(!t)return;
    ...
    if(t.id==='rtestMic')toggleRecording();else if(t.id==='rtestAnalyze')analyze();else if(t.id==='rtestHintBtn')hint();else if(t.id==='rtestRetry')retry();else if(t.id==='rtestNext')next();else if(t.id==='rtestAgain')newSession();else if(t.dataset?.rrate)rate(t.dataset.rrate)
  }
  function install(){
    const page=$('recitationTest');if(!page||!$('rtestStart')||!$('rtestDirectBtn'))return setTimeout(install,160);
    if(document.documentElement.dataset.werdGuest106)return;document.documentElement.dataset.werdGuest106='1';
