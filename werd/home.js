...
(function(){
  ...
  function injectDashboard(){
    ...
    const hero=home.querySelector('.hero');const wrap=document.createElement('div');wrap.id='smartDashboard';wrap.innerHTML=`
      ...
      <div class="smart-head"><h3>وصول سريع</h3><span>أدواتك اليومية</span></div>
      <div class="smart-quick"><button data-smart-page="searchPage">⌕ البحث</button><button data-smart-page="listening">🎧 الاستماع</button><button data-smart-page="prayer">🕌 الصلاة</button><button data-smart-page="hisn">🤲 حصن المسلم</button><button data-smart-page="asma">✦ أسماء الله</button><button data-smart-page="more">☷ المزيد</button></div>`;
  ...
  function wire(){
    ...
    document.querySelectorAll('[data-smart-page]').forEach(b=>b.onclick=()=>{const p=b.dataset.smartPage;if(p==='searchPage'&&typeof openSearch==='function')openSearch();else go(p)});
  }
