...
(function(){
  ...
  function locationData(){
    try{return JSON.parse(localStorage.getItem('werd_prayer_location')||'null')}catch(_){return null}
  }
  ...
  function relativeTime(iso){
    ...
    if(min<1)return'تم التحديث الآن';
    if(min<60)return`آخر تحديث قبل ${min} د`;
    const h=Math.round(min/60);if(h<24)return`آخر تحديث قبل ${h} س`;
    const d=Math.round(h/24);return`آخر تحديث قبل ${d} يوم`;
  }
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .plp-summary{padding:16px 18px 13px;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center}.plp-location-main b{font-size:16px;display:block}.plp-location-main small{display:block;color:var(--muted);font-size:11px;margin-top:4px}
      ...
      @media(max-width:430px){.plp-head{padding:15px}.plp-pin{width:42px;height:42px;flex-basis:42px;border-radius:14px}.plp-summary{padding:14px 15px 12px}.plp-settings{padding:0 15px 14px;grid-template-columns:1fr}.plp-details-toggle,.plp-coords,.plp-alert-wrap,.plp-privacy{margin-left:15px;margin-right:15px}.plp-head #locatePrayerBtn{padding:0 11px;font-size:11px}}
    `;document.head.appendChild(s);
  }
  function updateSummary(){
    const loc=locationData(),title=document.getElementById('plpLocationName'),meta=document.getElementById('plpLocationMeta'),chip=document.getElementById('plpQuality');
    if(!title||!meta||!chip)return;
    if(!loc){title.textContent='لم يتم تحديد الموقع';meta.textContent='اسمح للموقع لعرض مواقيت دقيقة';chip.textContent='بانتظار الموقع';chip.className='plp-quality idle';return}
    const q=quality(loc.accuracy);title.textContent='موقع الجهاز محفوظ';meta.textContent=`${relativeTime(loc.updatedAt)} • دقة ${Math.round(loc.accuracy||0)} م`;chip.textContent=q.label;chip.className=`plp-quality ${q.kind}`;
  }
  function enhance(){
    injectStyle();
    ...
    card.innerHTML=`<div class="plp-head"><div class="plp-title"><span class="plp-pin"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="2.6"/></svg></span><div><b>الموقع وإعدادات الصلاة</b><small>لتخصيص المواقيت والتنبيهات بدقة</small></div></div><div id="plpLocateMount"></div></div><div class="plp-summary"><div class="plp-location-main"><b id="plpLocationName">موقع الجهاز</b><small id="plpLocationMeta">—</small></div><span id="plpQuality" class="plp-quality idle">بانتظار الموقع</span></div><button type="button" class="plp-details-toggle" id="plpDetailsToggle">⌄ عرض الإحداثيات</button><div class="plp-coords" id="plpCoords"><div id="plpLabelMount"></div><button type="button" id="plpHideCoords">إخفاء</button></div><div class="plp-settings"><div class="plp-field"><label>طريقة حساب المواقيت</label><div id="plpMethodMount"></div></div><div class="plp-field"><label>حساب وقت العصر</label><div id="plpSchoolMount"></div></div></div><div class="plp-alert-wrap" id="plpAlertMount"></div><div class="plp-privacy"><span class="plp-lock">🔒</span><div id="plpPrivacyMount"></div></div>`;
    ...
    locate.addEventListener('click',()=>{setTimeout(updateSummary,400);setTimeout(updateSummary,1800)});
