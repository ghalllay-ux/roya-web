...
(function(){
  ...
  async function resolveCity(loc){
    ...
    let result=null;
    // Prefer OSM because it exposes the actual locality separately from the province/region.
    try{
    ...
    if(!result){
      try{
        ...
        const r=await fetch(u,{cache:'no-store'});if(r.ok){
          ...
          // Never promote a region/province name to a city name.
  ...
  async function updateSummary(){
    ...
    if(!cachedName&&Number(loc.accuracy)<=1000){const city=await resolveCity(loc);const name=cityLabel(city);if(name){title.textContent=name;if(note){note.classList.remove('warn');note.innerHTML=`<b>الموقع الحالي:</b> ${name}<br><b>طريقة الحساب:</b> الخيار أدناه لا يغيّر مدينتك؛ «أم القرى - مكة المكرمة» اسم طريقة حساب فقط.`}}}
  }
  ...
  async function preciseLocate(ev){
    ...
    try{
      ...
      const city=await resolveCity(loc);const name=cityLabel(city);
  ...
  function enhance(){
    ...
    // Capture phase prevents prayer.js from reusing a cached five-minute geolocation result.
  ...
  function boot(){
    ...
    let tries=0;const t=setInterval(()=>{tries++;if(enhance()){clearInterval(t);restorePrayerPage()}else if(tries>40)clearInterval(t)},150)
  }
