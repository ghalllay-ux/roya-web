...
(function(){
  ...
  function makeClient(){
    ...
    const client={
      ...
      async play(){await claim(client);return native.play()},
      ...
      _settings(){return{src,rate,preload,playsInline}}
    };
    clients.add(client);return client;
  }

  function cloneEvent(type,base,client){try{const ev=new Event(type);Object.defineProperty(ev,'werdClient',{value:client,configurable:true});return ev}catch(e){return{type,target:client,currentTarget:client,originalEvent:base}}}
  function dispatch(client,type,base){for(const fn of client?._listeners?.get(type)||[]){try{fn.call(client,cloneEvent(type,base,client))}catch(e){console.error('Werd audio listener',e)}}}

  async function claim(client){
    if(owner===client)return;
    const previous=owner,settings=client._settings();
    if(previous&&!native.paused)native.pause();
    owner=client;
    native.preload=settings.preload||'metadata';native.playsInline=settings.playsInline!==false;native.playbackRate=Number(settings.rate)||1;
    const wanted=settings.src||'';
    if((native.currentSrc||native.src)!==wanted){native.src=wanted;if(wanted)native.load()}
    window.dispatchEvent(new CustomEvent('werd:audio-owner',{detail:{clientId:client.__werdAudioClient,previousClientId:previous?.__werdAudioClient||null}}));
  }

  ['play','pause','ended','timeupdate','error','loadedmetadata','durationchange','waiting','canplay','seeking','seeked'].forEach(type=>{
