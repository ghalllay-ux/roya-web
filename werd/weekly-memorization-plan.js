...
(function(){
  ...
  function forecast(){
    ...
    for(let i=0;i<7;i++){
      ...
      let weakCount=0;const usedWeak=[];while(weakCount<alloc.weak&&weakCursor<weak.length){const w=weak[weakCursor++];if(!scheduled.some(x=>x.id===w.id&&x.next<=date)){usedWeak.push(w);weakCount++}}
  ...
  function actualPace(){
    const ds=daily(),today=localDate();let fresh=0,planDays=0;
    ...
    const f=forecast()[0]?.fresh??baseAllocation(ds.settings.minutes,ds.settings.newGoal).fresh;return{pace:Math.max(.25,f||0.25),basis:'هدفك الحالي'}
  }
